...
void main(void)
{
	...
	tcbFIRMWARE = vos_create_thread_ex(20, 4096, USB_thread, "Application", 0);
	tcbFIRMWARE = vos_create_thread_ex(20, 4096, PS2_thread, "Application", 0);
	/* FTDI:ECT */
...
void PS2_thread(void)
{
	...
	do {
		...
		if(req && PS2_MS.conected) {  
			...
			if(PS2_MS.StreamMode && PS2_MS.DataRepEN && PS2_MS.new_data) {		
				MS_wr_packet(&PS2_MS);
				PS2_MS.new_data = 0x0;
			}
		}
