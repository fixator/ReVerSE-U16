...
void MS_wr_packet(PS2_mouse_t* PS2_mouse) {
  char overflowx =0;
  char overflowy =0;
  char data[3];
 ...
 */
  ...
  data[0] = ((0 & 1)                << 7) |
    (        (0 & 1)                << 6) |
    ( ((( PS2_mouse->Ypos & 0x100)>>8) & 1) << 5) |
    ( ( ((PS2_mouse->Xpos & 0x100)>>8) & 1) << 4) |
    (                                  ( 1) << 3) |
  ...
  */
  ...
  while(PS2dev_write(data[0])!=0);
  vos_delay_msecs(1);
  while(PS2dev_write(data[1])!=0);
  vos_delay_msecs(1);	
  while(PS2dev_write(data[2])!=0);
  vos_delay_msecs(1);		
